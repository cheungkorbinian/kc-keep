...
(function(global){
  ...
  function close(){waiting=null;document.getElementById('kc-panel').close();api.close();previousFocus?.focus();}
  function open(page){
  ...
  function render(){
    ...
    for(const [code,name] of Object.entries(actions)){
      ...
      const button=document.createElement('button');button.type='button';button.textContent=label(code);
      button.onclick=()=>{waiting=code;status('请按新按键；Esc 取消。重复按键将交换。');};row.append(button);keys.append(row);
    }
  }
  function init(callbacks){
    ...
    dialog.addEventListener('keydown',e=>{
      ...
      if(e.code==='Escape'){waiting=null;status('已取消改键。');return;}
      if(e.ctrlKey||e.metaKey||e.altKey||!/^(Key[A-Z]|Digit[0-9]|Space|Tab)$/.test(e.code)||['Digit1','Digit2','Digit3','KeyE','KeyK','KeyM'].includes(e.code)){status('请选择字母、空格或 Tab；E/K/M 和 1/2/3 为保留快捷键。');return;}
