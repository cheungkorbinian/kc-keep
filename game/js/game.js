...
function spawnPickup(x, y, kind) {
  ...
  function packStructure(obj) {
    const { img, pair, ...data } = obj;
    if (img) data.imageKey = Object.keys(imgs).find((key) => imgs[key] === img);
    return data;
  }

  function unpackStructure(data) {
    const { imageKey, ...obj } = data;
    if (imageKey) obj.img = imgs[imageKey];
    return obj;
  }
  ...
  function validateSave(data) {
    ...
    if(data.v===3){
      ...
      if(w.fires.some(f=>!Array.isArray(f)||f.length!==4||!f.every(Number.isFinite)))return false;
    }
    return true;
  }
  ...
  function importSaveText(text, preserveBackup=true) {
    ...
    try {
      ...
      if(old){localStorage.setItem(SAVE_KEY,old);loadGame();}
      else localStorage.removeItem(SAVE_KEY);
      state=STATE.PAUSE;return '导入失败，已保留原存档：'+e.message;
    }
  }
